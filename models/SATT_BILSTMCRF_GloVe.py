...
'''
   This python file mainly contains code for training, testing and evaluation of Pre-trained model based AKE models for self-attention based Bi-LSTM+CRF adding GloVe Embeddings.
   The main function is satt_blcrf_glove(), called from main().
      1* Through the TextDataSet (Dataset): build the dictionary for vovabulary and cognitive signals.
      2* Build SATT_BiLSTM_CRF_GL() model
...
# Define SATT_BiLSTM_CRF_GL() model
class SATT_BiLSTM_CRF_GL(nn.Module):

    def __init__(self, vocab_size, embed_dim, hidden_dim,
                 num_layers=1, num_tags=6):
        super(SATT_BiLSTM_CRF_GL, self).__init__()

        # load pretrained GloVe vectors
        pretrain_model = KeyedVectors.load_word2vec_format(word2vecFile, binary=False)
        ...
        # load GloVe vectors for known words
        weight_matrix = torch.zeros((vocab_size, embed_dim))
        for word, index in self.word2ids.items():
            if word in pretrain_model.vocab:
                weight_matrix[index] = torch.tensor(pretrain_model[word], dtype=torch.float)
            # Load the pre-trained word vector matrix into the weight matrix of the embedding layer
    ...
    def forward(self, inputs, is_training=True):
        ...
        crf_feats = self.tocrf(reps)                                              # linear layer
...
def satt_blcrf_glove():
    ...
    # Define the model
    model = SATT_BiLSTM_CRF_GL(vocab_size=vocab_size,
                            embed_dim=embed_dim,
